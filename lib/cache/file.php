 ...
 * @lastmodified $Date: 2013-01-07 06:07:24 +1100 (Mon, 07 Jan 2013) $
 ...
 * @subpackage cake.cake.libs.cache
 */
class WortifyCacheFile extends WortifyCacheEngine
{
    /**
     ...
     *                prefix = string prefix for filename, default => wortify_
    ...
    function read($key)
    {
        ...
        if (!empty($data) && !empty($this->settings['serialize'])) {
            ...
            if (is_array($data)) {
                WortifyLoad::load('WortifyUtility');
                $data = WortifyUtility::recursive('stripslashes', $data);
            }
