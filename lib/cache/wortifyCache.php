...
class WortifyCache
{
    ...
    {
        if (!class_exists('WortifyCache' . ucfirst($name))) {
     ...
     * @access public
     */
    function engine($name = 'memcache', $settings = array())
    {
        if (!$name) {
            return false;
        }
        $cacheClass = 'WortifyCache' . ucfirst($name);
        $_this =& WortifyCache::getInstance();
