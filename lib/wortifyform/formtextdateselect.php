...
class WortifyFormTextDateSelect extends WortifyFormText
{

    function WortifyFormTextDateSelect($caption, $name, $size = 15, $value = 0)
    {
        $value = !is_numeric($value) ? time() : intval($value);
        $value = ($value == 0) ? time() : $value;
        $this->WortifyFormText($caption, $name, $size, 25, $value);
    }
        ...
        if (is_object($GLOBALS['xoTheme'])) {
	        ...
	        if (!$included) {
	            ...
	            $GLOBALS['xoTheme']->addScript('','', '
	                ...
	                var el = wortifyGetElementById(id);
	                if (calendar != null) {
	                calendar.hide();
	                } else {
