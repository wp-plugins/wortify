...
class WortifyThemeForm extends WortifyForm
{
     ...
     *
     * $colspan = new WortifyFormDhtmlTextArea( '', 'key', $value, '100%', '100%' );
    ...
    function render()
    {
        ...
        $ret = '<form name="' . $ele_name . '" id="' . $ele_name . '" action="' . $this->getAction() . '" method="' . $this->getMethod() . '" onsubmit="return wortifyFormValidate_' . $ele_name . '();"' . $this->getExtra() . '>
            ...
            if (!is_object($ele)) {
                ...
                if (!$ele->getNocolspan()) {
                    ...
                    if (($desc = $ele->getDescription()) != '') {
                        $ret .= '<div class="wortify-form-element-help">' . $desc . '</div>';
                    }
