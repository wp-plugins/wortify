...
/**
 * Base class for all objects in the Wortify kernel (and beyond)
 */
class WortifyObject
{
    /**
        ...
        if (isset($key) && isset($this->vars[$key])) {
            switch ($this->vars[$key]['data_type']) {
                case XOBJ_DTYPE_UNICODE_ARRAY:
                    if (is_array($value)) {
                        $this->vars[$key]['value'] =& array_walk($value, "wortify_aw_decode");
                    } else {
                        $this->vars[$key]['value'] =& wortify_convert_decode($value);
                    }
                    break;
    ...
    function getValues($keys = null, $format = 's', $maxDepth = 1)
    {
        ...
        foreach ($keys as $key) {
            if (isset($this->vars[$key])) {
                if (is_object($this->vars[$key]) && is_a($this->vars[$key], 'WortifyObject')) {
                    if ($maxDepth) {
                ...
                switch (strtolower($format)) {
                    ...
                    default:
                        if (!is_array($ret)) {
                            ...
                            if (is_array($ret)) {
                                $ret = array_walk($ret, "wortify_aw_decode");
                            }
                        }
                        return $ret;
                        break 1;
    ...
    function loadFilters($method)
    {
        ...
        if (!$modules_active = WortifyCache::read('system_modules_active')) {
 ...
 * @copyright copyright &copy; 2000 The WORTIFY Project
 */
class WortifyObjectHandler
{
    /**
     * holds referenced to {@link WortifyDatabase} class object
     *
     * @var object
     * @see WortifyDatabase
     * @access protected
     ...
     *
     * @param object $db reference to the {@link WortifyDatabase} object
     * @access protected
     */
    function WortifyObjectHandler(&$db)
    {
        $this->db =& $db;
    }
 ...
 *
 * @author Taiwen Jiang <phppp@users.sourceforge.net>
 * @author Jan Keller Pedersen <mithrandir@wortify.org>
 * @copyright copyright (c) The WORTIFY project
 * @package Kernel
 */

class WortifyPersistableObjectHandler extends WortifyObjectHandler
{
    /**
     ...
     *
     * @access protected
     * @param object $db {@link WortifyDatabase} object
     ...
     * @access protected
     */
    function WortifyPersistableObjectHandler($db = null, $table = '', $className = '', $keyName = '', $identifierName = '')
    {
    ...
    function setHandler($handler = null, $args = null, $path = null)
    {
        ...
        if (is_object($handler)) {
            ...
            $this->handler = WortifyModelFactory::loadHandler($this, $handler, $args, $path);
        }
        return $this->handler;
    }
     ...
     * @return object of handler {@link WortifyObjectAbstract}
     */
     ...
     * self::$handlers[$name] = WortifyModelFactory::loadHandler($this, $name, $args);
     * } else {
     ...
     *
     * To be enabled in WORTIFY 3.0 with PHP 5
     *
     * @access protected
    ...
    /**
     * *#@+
     * Methods of native handler {@link WortifyPersistableObjectHandler}
     */
    /**
     * create a new object
     *
     ...
     * @return object {@link WortifyObject}
     */
    ...
    /**
     * Load a {@link WortifyObject} object from the database
     *
     * @access protected
     * @param mixed $id ID
    ...
    /**
     * *#@+
     * Methods of write handler {@link WortifyObjectWrite}
     */
    /**
    ...
    /**
     * *#@+
     * Methods of read handler {@link WortifyObjectRead}
     */
    /**
     ...
     * @param bool $id_as_key use the ID as key for the array
     * @return array of objects/array {@link WortifyObject}
     */
     ...
     * @param array $fields variables to fetch
     * @param bool $asObject flag indicating as object, otherwise as array
     * @return array of objects     {@link WortifyObject}
     */
    ...
    /**
     * *#@+
     * Methods of stats handler {@link WortifyObjectStats}
     */
    /**
    ...
    /**
     * *#@+
     * Methods of joint handler {@link WortifyObjectJoint}
     */
    /**
    ...
    /**
     * *#@+
     * Methods of sync handler {@link WortifyObjectSync}
     */
    /**
