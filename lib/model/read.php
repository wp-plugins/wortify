...
class WortifyModelRead extends WortifyModelAbstract
{
    /**
    ...
    function getList($criteria = null, $limit = 0, $start = 0)
    {
        ...
        $Wortifyts =& WortifyTextSanitizer::getInstance();
        while ($myrow = $this->handler->db->fetchArray($result)) {
            // identifiers should be textboxes, so sanitize them like that
            $ret[$myrow[$this->handler->keyName]] = empty($this->handler->identifierName) ? 1 : $Wortifyts->htmlSpecialChars($myrow[$this->handler->identifierName]);
        }
        return $ret;
    }
    ...
    {
        $GLOBALS['wortifyLogger']->addDeprecated(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated, please use getAll instead.');
        if (isset($criteria) && is_subclass_of($criteria, "criteriaelement")) {
    ...
    {
        $GLOBALS['wortifyLogger']->addDeprecated(__CLASS__ . '::' . __FUNCTION__ . '() is deprecated.');
        $ret = array();
        while ($myrow = $this->handler->db->fetchArray($result)) {
