...
class Protector {
	...
	function output_log( $type = 'UNKNOWN' , $uid = 0 , $unique_check = false , $level = 1 )
	{
		...
		$loglevel = WortifyConfig::get('wortify_log_level');
	...
	{
		if( ! empty( $GLOBALS['wortifyOption']['nocommon'] ) || defined( '_LEGACY_PREVENT_EXEC_COMMON_' ) || defined( '_LEGACY_PREVENT_LOAD_CORE_' ) ) return ; // skip
		...
		$wo_server = WortifyConfig::get('wortify_dblayertrap_wo_server');
		if( empty( $wo_server ) ) {
			$this->_dblayertrap_check_recursive( $_SERVER ) ;
		}
	
		if( ! empty( $this->_dblayertrap_doubtfuls ) || $force_override ) {
			@define('DB_ALTERNATIVE' , 'ProtectorMysqlDatabase' ) ;
	...
	function stopforumspam($uid)
	{
	    ...
	    $query = "f=serial&ip=" . $this->getIP(true);
	    $query .= isset($_POST['email']) ? "&email=" . (isset($_POST['email'])?$_POST['email']:$_POST['user_login']) : '';
	    $query .= isset($_POST['uname']) ? "&username=" . (isset($_POST['uname'])?$_POST['uname']:$_POST['user_login']) : '';
	    ...
	    switch (WortifyConfig::get('wortify_stopforumspam_action')) {
	        default :
	        case 'log' :
	            break;
	        case 'san' :
	            $_POST = array();
	            $this->message .= "POST deleted for IP:" . $this->getIP(true);
	            break;
	        case 'biptime0' :
	            $_POST = array();
	            $this->message .= "BAN and POST deleted for IP:" . $this->getIP(true);
	...
	function check_dos_attack( $uid = 0 , $can_ban = false )
	{
		...
		if( $f5_count > WortifyConfig::get('wortify_dos_f5count') ) {
			...
			switch( WortifyConfig::get('wortify_dos_f5action') ) {
				default :
				case 'exit' :
				...
				case 'none' :
					...
					return true ;
				case 'biptime0' :
					if( $can_ban ) $this->register_bad_ips( time() + WortifyConfig::get('wortify_banip_time0') ) ;
					break ;
				case 'bip' :
		...
		if( $crawler_count > WortifyConfig::get('wortify_dos_crcount') ) {
			...
			switch( WortifyConfig::get('wortify_dos_craction') ) {
				default :
				case 'exit' :
					$this->output_log( $this->last_error_type , $uid , true , 16 ) ;
					exit ;
	...
	function disable_features()
	{
		...
		//
		// bit 11 : XSS+CSRFs in WORTIFY < 2.0.10
		//
