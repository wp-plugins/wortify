...
class WortifyAuthRest_Json extends WortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		...
		$data = file_get_contents(sprintf(WORTIFY_REST_API, 'xoops_authentication', http_build_query(array("username"=> $this->json_wortify_username, "password"=> $this->json_wortify_password, "auth" => array('username' => $uname, "password" => $pwd, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pwd), "rand"=>$rnd)))));
	...
	function validate($uname, $email, $pass, $vpass){
		$rnd = rand(-100000, 100000000);	
		$data = file_get_contents(sprintf(WORTIFY_REST_API, 'xoops_user_validate', http_build_query(array("username"=> $this->json_wortify_username, "password"=> $this->json_wortify_password, "validate" => array('uname' => $uname, "pass" => $pass, "vpass" => $vpass, "email" => $email, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pass), "rand"=>$rnd)))));
			$result = $this->obj2array(json_decode($data));	
		if ($result['ERRNUM']==1){
			return $result["RESULT"];
		} else {
	...
	function network_disclaimer(){
		...
		$data = file_get_contents(sprintf(WORTIFY_REST_API, 'xoops_network_disclaimer', http_build_query(array("username"=> $this->json_wortify_username, "password"=> $this->json_wortify_password))));
			$result = $this->obj2array(json_decode($data));	
