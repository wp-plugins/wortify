...
class WortifytsFlash extends WortifyTextSanitizerExtension
{
    function encode($textarea_id)
    {
        $config = parent::loadConfig(dirname(__FILE__));
        $code = "<img src='{$this->image_path}/swf.gif' alt='" . _WORTIFY_FORM_ALTFLASH . "' onclick='wortifyCodeFlash(\"{$textarea_id}\",\"" . htmlspecialchars(_WORTIFY_FORM_ENTERFLASHURL, ENT_QUOTES) . "\",\"" . htmlspecialchars(_WORTIFY_FORM_ALT_ENTERHEIGHT, ENT_QUOTES) . "\",\"" . htmlspecialchars(_WORTIFY_FORM_ALT_ENTERWIDTH, ENT_QUOTES) . "\", \"" . $config['detect_dimension'] . "\");'  onmouseover='style.cursor=\"hand\"'/>&nbsp;";
        $javascript = <<<EOF
            function wortifyCodeFlash(id, enterFlashPhrase, enterFlashHeightPhrase, enterFlashWidthPhrase, enableDimensionDetect)
            {
