 ...
 * This File:	auth_wgetserialised_provisionning.php		
 * Description:	Auth Provisionning Library for wGET Serialised API Package
 * Date:		09/09/2012 19:34 AEST
 * License:		GNU3
 * 
 */

class WortifyAuthRest_WgetserialisedProvisionning {
	...
	function &getInstance(&$auth_instance)
	{
		...
		if (!isset($provis_instance)) {
			$provis_instance = new WortifyAuthRest_WgetserialisedProvisionning($auth_instance);
		}
	 ...
	 */
	function WortifyAuthRest_WgetserialisedProvisionning (&$auth_instance) {
	...
	function change_json(&$wortifyUser, $datas, $uname, $pwd = null) {	
		$ret = false;
		$member_handler =& wortify_gethandler('member');
		$wortifyUser->setVar('pass', md5(stripslashes($pwd)));
		$tab_mapping = explode('|', $this->json_field_mapping);
