 ...
 * This File:	auth_wgetxml_provisionning.php		
 * Description:	Auth Provisionning Library for wGET XML API Package
 * Date:		09/09/2012 19:34 AEST
 * License:		GNU3
 * 
 */

class WortifyAuthRest_WgetxmlProvisionning {
	...
	function &getInstance(&$auth_instance)
	{
		...
		if (!isset($provis_instance)) {
			$provis_instance = new WortifyAuthRest_WgetxmlProvisionning($auth_instance);
		}
	 ...
	 */
	function WortifyAuthRest_WgetxmlProvisionning (&$auth_instance) {
	...
	function add($datas, $uname, $pwd = null) {
		...
		if ($this->xml_provisionning)
			$tab_mapping = explode('|', $this->xml_field_mapping);
		else 
		...
		if ($member_handler->insertUser($newuser)) {
			foreach ($this->xml_provisionning_group as $groupid)
	...
	function change_xml(&$wortifyUser, $datas, $uname, $pwd = null) {	
		$ret = false;
		$member_handler =& wortify_gethandler('member');
		$wortifyUser->setVar('pass', md5(stripslashes($pwd)));
		$tab_mapping = explode('|', $this->xml_field_mapping);
