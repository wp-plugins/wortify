 ...
 */
class WortifytsTextfilter extends WortifyTextSanitizerExtension
{
    function load(&$ts, $text, $force = false)
    {
        global $wortifyUser, $wortifyConfig, $wortifyUserIsAdmin;
        if (empty($force) && $wortifyUserIsAdmin) {
            return $text;
        }
        ...
        if (wortify_load("purifier", "framework")) {
            $text = WortifyPurifier::purify($text);
            return $text;
        }
