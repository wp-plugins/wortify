...
class WortifytsSyntaxhighlight extends WortifyTextSanitizerExtension
{
    function load(&$ts, $source, $language)
    {
        ...
        if ($config['highlight'] == 'geshi') {
            ...
            if ($source2 = WortifytsSyntaxhighlight::geshi($source, $language)) {
                return $source2;
            }
        }
        $source = WortifytsSyntaxhighlight::php($source);
        return $source;
    }
    ...
    {
        if (!@wortify_load("geshi", "framework")) {
            return false;
        }

        // Create the new WortifyGeshi object, passing relevant stuff
        // WortifyGeshi should be extending geSHi in Frameworks/geshi/wortifygeshi.php
        $geshi = new WortifyGeshi($source, $language);
